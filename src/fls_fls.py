...
def calculate_scene(state):
    ...
    arc2_global = (R @ arc2_local.T).T + s2_origin

    return p1_local, fov1_local, arc1_local, s2_origin, fov2_global, arc2_global
...
# Figure to visualize the sonar FOVs and the intersecting elevation arcs
fig_viz = plt.figure(figsize=(10, 8), num="3D Visualization")
ax = fig_viz.add_subplot(111, projection='3d')

def update_plot():
    ax.clear()
    p1, fov1, arc1, s2_org, fov2, arc2 = calculate_scene(state)
    ...
    # Plot feature point
    ax.scatter(p1[0], p1[1], p1[2], color='r', s=15, label='Feature')
    
    # Set up subtitles that will display the feature location, and relative pose values
    title_coords = f"Feature in S1 Frame: x={p1[0]:.2f}, y={p1[1]:.2f}, z={p1[2]:.2f}"
...
# Figure to use for paremeter controls
...
def add_control_row(y_start, label, key_prefix, color_text):
    ...
    # Return both TextBoxes and Sliders
    return [tb_min, tb_max, tb_az, tb_el], [sl_fr, sl_faz, sl_fel]

# Add S1 controls
tbs1, sls1 = add_control_row(0.92, "Sonar 1 (Blue)", 's1', 'blue')

# Add S2 controls
tbs2, sls2 = add_control_row(0.60, "Sonar 2 (Orange)", 's2', 'darkorange')
