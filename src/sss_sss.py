...
def calculate_scene(state):
    ...
    # Second SSS Feature 3D Ambiguity - we know range, but unknown azimuth and elevation
    r1 = np.linalg.norm(p1_local)
    sss1_surface_local = ut.generate_azimuth_elevation_surface_lines(r1, state['s1_az'], state['s1_el'], density=8)
    ...
    sss2_surface_local = ut.generate_azimuth_elevation_surface_lines(r2, state['s2_az'], state['s2_el'], density=8)
    ...
    sss2_surface_global = []
    for line in sss2_surface_local:
        line_global = (R @ line.T).T + s2_origin
        sss2_surface_global.append(line_global)

    return p1_local, fov1_local, sss1_surface_local, s2_origin, fov2_global, sss2_surface_global

# --- Global State ---
state = {
    # First SSS Params
    's1_min_r': 1.0, # min range (meters)
    's1_max_r': 15.0, # max range (meters)
    's1_az': 150.0, # azimuth aperture (degrees)
    's1_el': 3.0, # elevation aperture (degrees)
    ...
    's2_az': 150.0, # azimuth aperture (degrees)
...
fig_viz = plt.figure(figsize=(10, 8), num="SSS & SSS Visualization")
ax = fig_viz.add_subplot(111, projection='3d')

def update_plot():
    ax.clear()
    p1, fov1, sss1_surface, s2_org, fov2, sss2_surface = calculate_scene(state)
    # Plot First SSS (Blue)
    ax.scatter([0], [0], [0], color='black', s=20, label='SSS 1 Origin')
    for line in fov1:
    ...
    for i, line in enumerate(sss1_surface):
        # Only label the first line to avoid cluttering legend
        ax.plot(line[:,0], line[:,1], line[:,2], color='b', alpha=0.6, linewidth=1)
    # Plot Second SSS (Orange)
    ax.scatter(s2_org[0], s2_org[1], s2_org[2], color='darkorange', s=20, label='SSS 2 Origin')
    for line in fov2:
        ax.plot(line[:,0], line[:,1], line[:,2], color='orange', linestyle='--', alpha=0.3)
    # Plot SSS Ambiguity Surface (Grid)
    # We iterate through the lines that make up the surface
    for i, line in enumerate(sss2_surface):
        # Only label the first line to avoid cluttering legend
    ...
    title_coords = f"Feature in SSS 1 Frame: x={p1[0]:.2f}, y={p1[1]:.2f}, z={p1[2]:.2f}"
    ...
    ax.set_title(f"Cross-Modal Geometry (SSS to SSS)\n{title_coords}\n{title_pose}", fontsize=10)
...
# Add SSS 1 controls
tbs1, sls1 = add_control_row(0.92, "Sidescan 1 (Blue)", 's1', 'blue')

# Add SSS 2 controls
tbs2, sls2 = add_control_row(0.60, "Sidescan 2 (Orange)", 's2', 'darkorange')

# Add rotation controls
y_rot = 0.25
plt.text(0.05, y_rot, "Relative Rotation", fontsize=12, weight='bold', transform=fig_controls.transFigure)
